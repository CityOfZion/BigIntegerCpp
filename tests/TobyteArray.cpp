#include <variant>
#include <vector>
#include <math.h>
#include "../src/BigInteger.h"
#include "gtest/gtest.h"

TEST(byte_array, int_byte_array)
{
    typedef std::tuple<int, byte_array> TestTup;
    typedef std::vector<TestTup> TestVec;
    TestVec data;

    int intMinValue = std::numeric_limits<int>::min();
    int intMaxValue = std::numeric_limits<int>::max();

    data.emplace_back(TestTup( 0, { 0x00 }));
    data.emplace_back(TestTup( 3, { 0x03 }));
    data.emplace_back(TestTup( 128, { 0x80, 0x00 } ));
    data.emplace_back(TestTup( 200, { 0xc8, 0x00 } ));
    data.emplace_back(TestTup( 256, { 0x00, 0x01 } ));
    data.emplace_back(TestTup( 2005, { 0xd5, 0x07 } ));
    data.emplace_back(TestTup( 10197, { 0xd5, 0x27 } ));
    data.emplace_back(TestTup( 33023, { 0xff, 0x80, 0x00 } ));
    data.emplace_back(TestTup( 2368349, { 0x5d, 0x23, 0x24 } ));
    data.emplace_back(TestTup( 10756957, { 0x5d, 0x23, 0xa4, 0x00 } ));
    data.emplace_back(TestTup( 193100307, { 0x13, 0x7a, 0x82, 0x0b } ));
    data.emplace_back(TestTup( 1266842131, { 0x13, 0x7a, 0x82, 0x4b } ));
    data.emplace_back(TestTup( intMaxValue, { 0xff, 0xff, 0xff, 0x7f } ));
    data.emplace_back(TestTup( -1, { 0xff } ));
    data.emplace_back(TestTup( -128, { 0x80 } ));
    data.emplace_back(TestTup( -172, { 0x54, 0xff } ));
    data.emplace_back(TestTup( -23439, { 0x71, 0xa4 } ));
    data.emplace_back(TestTup( -51301, { 0x9b, 0x37, 0xff } ));
    data.emplace_back(TestTup( -126341, { 0x7b, 0x12, 0xfe } ));
    data.emplace_back(TestTup( -13194515, { 0xed, 0xaa, 0x36, 0xff } ));
    data.emplace_back(TestTup( -2068145902, { 0x12, 0x99, 0xba, 0x84 } ));
    data.emplace_back(TestTup( intMinValue, { 0x00, 0x00, 0x00, 0x80 } ));

    for (auto elem : data)
    {
        BigInteger bi = BigInteger(std::get<0>(elem));
        byte_array bytes = bi.to_byte_array();
        ASSERT_EQ( bytes, std::get<1>(elem) );

        BigInteger bi2 = BigInteger(bytes);
        ASSERT_EQ( bi, bi2 );
    }
}

 TEST(byte_array, long_byte_array)
 {
     typedef std::tuple<int64_t , byte_array> TestTup;
     typedef std::vector<TestTup> TestVec;
     TestVec data;

     int64_t llongMinValue = std::numeric_limits<int64_t>::min();
     int64_t llongMaxValue = std::numeric_limits<int64_t>::max();

     data.emplace_back(TestTup(static_cast<int64_t>(0x100112233), { 0x33, 0x22, 0x11, 0x00, 0x01 }));
     data.emplace_back(TestTup(static_cast<int64_t>(0x300112233), { 0x33, 0x22, 0x11, 0x00, 0x03 }));
     data.emplace_back(TestTup(static_cast<int64_t>(0x8000112233), { 0x33, 0x22, 0x11, 0x00, 0x80, 0x00 }));
     data.emplace_back(TestTup(static_cast<int64_t>(0x3cd00112233), { 0x33, 0x22, 0x11, 0x00, 0xcd, 0x03 }));
     data.emplace_back(TestTup(static_cast<int64_t>(0xf92100112233), { 0x33, 0x22, 0x11, 0x00, 0x21, 0xf9, 0x00 }));
     data.emplace_back(TestTup(static_cast<int64_t>(0x749aa00112233), { 0x33, 0x22, 0x11, 0x00, 0xaa, 0x49, 0x07 }));
     data.emplace_back(TestTup(static_cast<int64_t>(0x80112200112233), { 0x33, 0x22, 0x11, 0x00, 0x22, 0x11, 0x80, 0x00 }));
     data.emplace_back(TestTup(static_cast<int64_t>(0x7654321000112233), { 0x33, 0x22, 0x11, 0x00, 0x10, 0x32, 0x54, 0x76 }));
     data.emplace_back(TestTup(llongMaxValue, { 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x7f }));
     data.emplace_back(TestTup(static_cast<int64_t>(-0x100112233), { 0xcd, 0xdd, 0xee, 0xff, 0xfe }));
     data.emplace_back(TestTup(static_cast<int64_t>(-0x8000112233), { 0xcd, 0xdd, 0xee, 0xff, 0x7f, 0xff }));
     data.emplace_back(TestTup(static_cast<int64_t>(-0x3cd00112233), { 0xcd, 0xdd, 0xee, 0xff, 0x32, 0xfc }));
     data.emplace_back(TestTup(static_cast<int64_t>(-0xf92100112233), { 0xcd, 0xdd, 0xee, 0xff, 0xde, 0x06, 0xff }));
     data.emplace_back(TestTup(static_cast<int64_t>(-0x749aa00112233), { 0xcd, 0xdd, 0xee, 0xff, 0x55, 0xb6, 0xf8 }));
     data.emplace_back(TestTup(static_cast<int64_t>(-0x80112200112233), { 0xcd, 0xdd, 0xee, 0xff, 0xdd, 0xee, 0x7f, 0xff }));
     data.emplace_back(TestTup(static_cast<int64_t>(-0x7654321000112233), { 0xcd, 0xdd, 0xee, 0xff, 0xef, 0xcd, 0xab, 0x89 }));
     data.emplace_back(TestTup(llongMinValue, { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80 }));
     data.emplace_back(TestTup(static_cast<int64_t>(-0x100000000), { 0x00, 0x00, 0x00, 0x00, 0xff }));
     data.emplace_back(TestTup(static_cast<int64_t>(-0x300000000), { 0x00, 0x00, 0x00, 0x00, 0xfd }));
     data.emplace_back(TestTup(static_cast<int64_t>(-0x8000000000), { 0x00, 0x00, 0x00, 0x00, 0x80 }));
     data.emplace_back(TestTup(static_cast<int64_t>(-0xfe00000000), { 0x00, 0x00, 0x00, 0x00, 0x02, 0xff }));
     data.emplace_back(TestTup(static_cast<int64_t>(-0xff00000000), { 0x00, 0x00, 0x00, 0x00, 0x01, 0xff }));
     data.emplace_back(TestTup(static_cast<int64_t>(-0xfeff00000000), { 0x00, 0x00, 0x00, 0x00, 0x01, 0x01, 0xff }));
     data.emplace_back(TestTup(static_cast<int64_t>(-0xffff00000000), { 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0xff }));
     data.emplace_back(TestTup(static_cast<int64_t>(-0xfeffff00000000), { 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x01, 0xff }));
     data.emplace_back(TestTup(static_cast<int64_t>(-0xffffff00000000), { 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0xff }));

     for (auto elem : data)
     {
         BigInteger bi = BigInteger(std::get<0>(elem));
         byte_array bytes = bi.to_byte_array();
         ASSERT_EQ( bytes, std::get<1>(elem) );

         BigInteger bi2 = BigInteger(bytes);
         ASSERT_EQ( bi, bi2 );
     }
 }

TEST(byte_array, string_byte_array)
{
    typedef std::tuple<std::string, byte_array> TestTup;
    typedef std::vector<TestTup> TestVec;
    TestVec data;

    data.emplace_back(TestTup( "0", { 0 }));
    data.emplace_back(TestTup( "127", { 0x7F }));
    data.emplace_back(TestTup( "128", { 0x00, 0x80 }));
    data.emplace_back(TestTup( "33022", { 0x00, 0x80, 0xFE }));
    data.emplace_back(TestTup( "-32514", { 0x80, 0xFE }));
    data.emplace_back(TestTup( "-18374686475376656384", { 0xff, 0x01, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00 }));
    data.emplace_back(TestTup( "-18446744069414584320", { 0xff, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00 }));
    data.emplace_back(TestTup( "", {  }));
    data.emplace_back(TestTup( "-", {  }));

    for (auto elem : data)
    {
        BigInteger bi = BigInteger::parse(std::get<0>(elem));
        byte_array bytes = bi.to_byte_array();
        std::reverse(bytes.begin(), bytes.end());
        ASSERT_EQ( bytes, std::get<1>(elem) );

        std::reverse(bytes.begin(), bytes.end());
        BigInteger bi2 = BigInteger(bytes);
        ASSERT_EQ( bi, bi2 );
    }
}
